...
class Origin(object):
    ...
    @classmethod
    def loads(cls, data):
        username, sess_id, sess_version, nettype, addrtype, unicast_address = re.split(
            b" +", data, 5)
        sess_id = int(sess_id)
...
class SDP(object):
    ...
    _once = ["u", "c"]
    _multi = []
    _attributes_allowed = [b"v", b"o", b"s", b"i", b"u",
                           b"e", b"p", b"c", b"b", b"t", b"r", b"z", b"a", b"m"]
    ...
    def loads(cls, data):
        ...
        for line in data.split(b"\n"):
            try:
                ...
                logger.warning(
                    "Can't parse sdp data: '{}':  {!s}".format(repr(line)[:128], error_msg))
