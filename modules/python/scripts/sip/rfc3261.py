...
class Message(object):
    ...
    def create_response(self, code, message = None, personality = None):
        logger.info(
            "Creating Response: code={}, message={}".format(code, message))
    ...
    def dumps(self):
        ...
        if self.method != None:
            h.append(
                self.method + b" " + self.uri.dumps() + b" " + self.protocol)
        elif self.response_code != None:
            h.append(self.protocol + b" " +
                     int2bytes(self.response_code) + b" " + self.status_message)
        else:
            return None
        ...
        if self.sdp != None:
            sdp = self.sdp.dumps()
            self.headers.append(
                Header(name = b"content-type", value = b"application/sdp"))
            self.headers.append(
                Header(name = b"content-length", value = len(sdp)))
    ...
    def headers_exist(self, headers, overwrite = False):
        if overwrite == False:
            headers = headers + \
                [b"to", b"from", b"call-id", b"cseq", b"contact"]
    ...
    def loads(cls, data):
        ...
        try:
            ...
            logger.warning("Can't parse first line of sip message: {}".format(
                repr(headers_data[0])[:128]))
                ...
                if content_type != None and content_type.value.lower().strip() == b"application/sdp":
                    try:
                        ...
                        raise ErrorWithResponse(
                            msg, BAD_REQUEST, "Invalid SIP body")

                l = l + content_length
            else:
                logger.info(
                    "Body is to short than the given content-length: Content-Length {}, Body {}".format(content_length, len(body)))
...
class Via(object):
    ...
    _syntax = re.compile(
        b"SIP */ *2\.0 */ *(?P<protocol>[a-zA-Z]+) *(?P<address>[^ :;]*) *(:(?P<port>[0-9]+))?( *; *(?P<params>.*))?")
