...
class User(object):
    def __init__(self, user_id, **kwargs):
        ...
        self.expire_time = datetime.datetime.now(), datetime.datetime.now() + \
            datetime.timedelta(0, self.expires)
...
class RtpUdpStream(connection):
    ...
    def handle_io_in(self, data):
        ...
        if self._pcap != None:
            self._pcap.write(
                src_port = self.remote.port, dst_port = self.local.port, data = data)
...
class SipCall(connection):
    ...
    """
    CLOSED, SESSION_SETUP, ACTIVE_SESSION, SESSION_TEARDOWN, INVITE, INVITE_TRYING, INVITE_RINGING, INVITE_CANCEL, CALL = range(
        9)
    ...
    def __handle_invite(self, watcher, events):
        ...
        if self.__state == SipCall.INVITE_TRYING:
            ...
            logger.info("Choosing ring delay between {} and {} seconds: {}".format(
                self._user.pickup_delay_min, self._user.pickup_delay_max, delay))
        ...
        if self.__state == SipCall.INVITE_RINGING:
            ...
            media_port_names = g_sipconfig.get_sdp_media_port_names(
                self._user.sdp)
    ...
    def close(self):
        logger.debug("{!s} close".format(self))
        logger.debug(
            "SipCall.close {} Session {}".format(self, self.__session))
    ...
    def handle_ACK(self, msg_ack):
        ...
        if cseq.seq != cseq_ack.seq:
            logger.info("Sequence number doesn't match INVITE id {}; ACK id {}".format(
                cseq.seq, cseq_ack.seq))
            return
    ...
    def handle_CANCEL(self, msg_cancel):
        ...
        if cseq.seq != cseq_cancel.seq:
            logger.info("Sequence number doesn't match INVITE id {}; CANCEL id {}".format(
                cseq.seq, cseq_cancel.seq))
            return
...
class SipSession(connection):
    ...
    def __init__(self, proto = None):
        ...
        self.personality = g_sipconfig.get_personality_by_address(
            self.local.host)

        logger.info(
            "SIP Session created with personality '{}'".format(self.personality))
        self._auth = None
        self._state = None
    ...
    def handle_io_in(self, data):
        ...
        if self.transport == "udp":
            ...
            logger.debug(
                "Got {} bytes, Used {} bytes".format(len(data), len_used))
    ...
    def _handle_ABC(self, msg):
        ...
        if call_id not in g_call_ids or g_call_ids[call_id] == None:
            logger.warn(
                "{!s} request does not match any existing SIP session".format(handler_name))
            icd = incident("dionaea.modules.python.sip.command")
            icd.con = self
            msg_to_icd(msg,d=icd)
            icd.report()
            self.send(
                msg.create_response(rfc3261.CALL_TRANSACTION_DOSE_NOT_EXIST).dumps())
            return
    ...
    def handle_INVITE(self, msg):
        ...
        if call_id in g_call_ids and g_call_ids[call_id] == None:
            logger.warn(
                "SIP session with Call-ID {} already exists".format(call_id[:128]))
            # ToDo: error
            return
    ...
    def handle_OPTIONS(self, msg):
        ...
        res.headers.append(
            rfc3261.Header(name = "Accept-Language", value = "en"))
