...
class SipConfig(object):
    ...
    def __init__(self, config=None):
        ...
        self.users = os.path.join(
            self.root_path, config.get("users", "var/dionaea/sipaccounts.sqlite"))
        ...
        if not self._table_exists("users"):
            self._cur.execute(
                "CREATE TABLE IF NOT EXISTS users (username STRING, password STRING, personality STRING, pickup_delay_min INTEGER, pickup_delay_max INTEGER, action STRING, sdp STRING)")
            # example without password
            self._cur.execute(
                "INSERT INTO users (username, password, personality, pickup_delay_min, pickup_delay_max, action, sdp) VALUES ('^[0-9]{1,12}$', '', 'default', 5, 10, 'default', 'default')")
            # example with password
            self._cur.execute(
                "INSERT INTO users (username, password, personality, pickup_delay_min, pickup_delay_max, action, sdp) VALUES ('^pw[0-9]{1,12}$', 'password', 'default', 5, 10, 'default', 'default')")

        if not self._table_exists("sdp"):
            self._cur.execute(
                "CREATE TABLE IF NOT EXISTS sdp (name STRING, sdp STRING)")
            self._cur.execute(
                "INSERT INTO sdp (name, sdp) VALUES ('default', ?)", (DEFAULT_SDP,))
    ...
    def get_user_by_username(self, personality, username):
        ...
        cur = conn.cursor()
        cur.execute(
            "SELECT username, password, pickup_delay_min, pickup_delay_max, action, sdp FROM users WHERE personality = ? AND ? REGEXP username", (personality, username))
        row = cur.fetchone()
    ...
    def get_sdp_by_name(self, name, media_ports, **params):
        ...
        """
        logger.debug("Loading sdp with: params = {}, media_ports {}".format(
            pprint.pformat(params), pprint.pformat(media_ports)))
    ...
    def get_sdp_media_port_names(self, name):
        ...
        media_ports = re.findall(
            "{(audio_port[0-9]*|video_port[0-9]*)}", data[0])
...
class PCAP(object):
    ...
    def open(self, msg_stack, **params):
        ...
        try:
            ...
            logger.warning(
                "Can't create RTP-Dump file: {}".format(os.path.join(path, filename)))
        ...
        for msg in msg_stack:
            ...
            self.write(ts = ts, tm = tm, src_ether = src_ether, src_host = src_host, src_port = src_port,
                       dst_ether = dst_ether, dst_host = dst_host, dst_port = dst_port, data = msg[1].dumps())
