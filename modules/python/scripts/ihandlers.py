...
def new():
    ...
    if "p0f" in g_dionaea.config()['modules']['python']['ihandlers']['handlers']:
        import dionaea.p0f
        g_handlers.append(
            dionaea.p0f.p0fhandler(g_dionaea.config()['modules']['python']['p0f']['path']))
    ...
    if "logxmpp" in g_dionaea.config()['modules']['python']['ihandlers']['handlers']:
        ...
        for client in g_dionaea.config()['modules']['python']['logxmpp']:
            ...
            x = dionaea.logxmpp.logxmpp(conf['server'], int(conf['port']), conf[
                                        'username'], conf['password'], resource, conf['muc'], conf['config'])
            g_handlers.append(x)
